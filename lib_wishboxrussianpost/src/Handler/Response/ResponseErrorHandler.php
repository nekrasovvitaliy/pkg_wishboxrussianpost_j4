 ...
 * @since 1.0.0
 */
class ResponseErrorHandler extends AbstractResponseHandler
{
	/**
	...
	protected function handleResponse(string $path, ResponseData $responseData): bool
	{
		if ($responseData->getCode() >= 400 && $responseData->getCode() < 500)
		{
			...
			if (is_object($responseBodyData)
				&& isset($responseBodyData->error))
			{
				$e = ApiExceptionFactory::createErrorException(
					$responseBodyData->error,
					$responseData
				);
				throw $e;
			}
